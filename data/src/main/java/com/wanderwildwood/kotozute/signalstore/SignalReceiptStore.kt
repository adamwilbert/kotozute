 ...
 *
 * This is only the record of what is owed. It holds no message and no content: a receipt is
 * the pair that identifies one, and that is all there is to keep.
 */
internal class SignalReceiptStore(private val db: ProtocolDatabase) {

    /** Somebody still waiting to be told their message arrived, and since when. */
     ...
     * measured from.
     */
    fun owe(recipient: String, timestamps: List<Long>, now: Long = System.currentTimeMillis()) =
        withStoreLock(db) {
            if (recipient.isBlank()) return@withStoreLock
            db.writableDatabase.compileStatement(
                """
                INSERT INTO receipt_owed (recipient, sent_timestamp, owed_since)
                VALUES (?, ?, ?)
                ON CONFLICT(recipient, sent_timestamp) DO NOTHING
                """.trimIndent()
            ).use { statement ->
    ...
    /** Who is still owed a receipt and still worth telling, grouped by person. */
    fun owed(now: Long = System.currentTimeMillis()): Map<String, List<Long>> = withStoreLock(db) {
        db.readableDatabase.rawQuery(
            "SELECT recipient, sent_timestamp, owed_since FROM receipt_owed ORDER BY owed_since",
            null
        ).use { c ->
    ...
    /** They have been told. */
    fun clear(recipient: String, timestamps: List<Long>) = withStoreLock(db) {
        if (recipient.isBlank() || timestamps.isEmpty()) return@withStoreLock
        db.writableDatabase.compileStatement(
            "DELETE FROM receipt_owed WHERE recipient = ? AND sent_timestamp = ?"
        ).use { statement ->
    ...
    fun abandonExpired(now: Long = System.currentTimeMillis()): Int = withStoreLock(db) {
        db.writableDatabase.compileStatement(
            ...
            statement.executeUpdateDelete().also { gone ->
                if (gone > 0) {
                    Timber.w(
                        "signal receipt: gave up telling %d sender(s) their message arrived",
                        gone
                    )
                }
            }
